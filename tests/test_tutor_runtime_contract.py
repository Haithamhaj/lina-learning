...
def test_tutor_turn_v12_requires_nullable_visual_order_without_rewriting_other_metadata() -> None:
    ...
    visual_order = TUTOR_OUTPUT_JSON_SCHEMA["properties"]["workspace_visual_order"]
    assert visual_order["anyOf"][0]["additionalProperties"] is False
    assert visual_order["anyOf"][0]["properties"]["version"]["enum"] == ["workspace-visual-order-v1"]
    assert visual_order["anyOf"][0]["properties"]["pattern"]["enum"] == ["PROCESS"]
    assert visual_order["anyOf"][0]["properties"]["required_semantics"]["minItems"] == 2
...
def test_primary_tutor_contract_instructs_workspace_intent_and_local_provider_returns_null() -> None:
    ...
    assert result.output["canvas_brief"] is None
    assert result.output["canvas_change_intent"] is None


def test_tutor_v12_requires_nullable_canvas_brief_and_keeps_workspace_intent() -> None:
    from services.tutor.candidate_events import TUTOR_OUTPUT_JSON_SCHEMA, TUTOR_OUTPUT_RESPONSE_SCHEMA
...
def test_tutor_payload_preserves_complete_within_budget_visual_personalization_catalog() -> None:
    payload = build_tutor_model_payload(
        ...
        visual_personalization_catalog=[
            {"fact_key": "favorite:butterflies", "category": "FAVORITE", "display_statement": "Likes butterflies"},
            {"fact_key": "activity:garden", "category": "ACTIVITY", "display_statement": "Gardens"},
            {"fact_key": "pet:cat", "category": "PET", "display_statement": "Has a cat"},
            {"fact_key": "preference:space", "category": "PREFERENCE", "display_statement": "Likes space"},
        ],
    )

    assert [item["fact_key"] for item in payload["visual_personalization_catalog"]] == [
        "favorite:butterflies", "activity:garden", "pet:cat", "preference:space",
    ]
    assert "preference:space" in payload["input"]
