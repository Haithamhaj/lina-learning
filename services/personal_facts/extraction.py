    ...
    *,
    ...
    return {
        "instructions": (
            ...
            "For ADD_NEW, fact_key must always be lowercase ASCII in <category-prefix>:<stable-key> form. "
            "Use the matching prefix: PREFERENCE -> preference:, FAVORITE -> favorite:, ACTIVITY -> activity:, "
            "PET -> pet:, RELATIONSHIP -> relationship:, SAFE_PERSONAL_CONTEXT -> context:. "
            "fact_key identifies the stable fact type, not the full natural-language sentence. "
            "Example: I like drawing -> category=PREFERENCE, fact_key=preference:drawing, value=LIKE. "
            "Example: I have a cat named Mishmish -> category=PET, fact_key=pet:cat, value=Mishmish. "
            "display_statement may remain natural Arabic or English. supporting_assertions must remain exact grounded excerpts from Student messages. "
