...
from services.studio.subjects.agentic_canvas import PROFILE_VERSION as AGENTIC_CANVAS_PROFILE_VERSION, make_legacy_profile as make_legacy_agentic_canvas_profile, make_profile as make_agentic_canvas_profile, make_v2_profile as make_v2_agentic_canvas_profile
...
from services.studio.subjects.decimal_place_value import PROFILE_VERSION as PLACE_PROFILE_VERSION, make_profile as make_place_profile


PRODUCTION_CURRENT_PROFILE_VERSIONS = MappingProxyType({
    "MATH": PLACE_PROFILE_VERSION,
    "SCIENCE": SCIENCE_PROFILE_VERSION,
    "ENGLISH": ENGLISH_PROFILE_VERSION,
    "ARABIC": ARABIC_PROFILE_VERSION,
    "CANVAS": AGENTIC_CANVAS_PROFILE_VERSION,
})
...
def production_subject_registry() -> SubjectCapabilityRegistry:
    ...
    return SubjectCapabilityRegistry(
        (*baseline_profiles, make_visual_profile(), make_production_process_profile(), make_canvas_production_profile(), make_legacy_agentic_canvas_profile(), make_v2_agentic_canvas_profile(), make_agentic_canvas_profile(), make_ten_profile(), make_profile(), make_place_profile(), make_process_sequence_profile(), make_sentence_ordering_profile(), make_arabic_sentence_ordering_profile())
    )
