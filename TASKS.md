...
| **CTX-03** — Hybrid Segment Context Runtime | 5 | IN PROGRESS / CTX-03A ACCEPTED / CTX-03B ACCEPTED / CTX-03C READY | Current-Segment-only runtime: session-local Segment lineage and compact Structured Segment State are accepted; CTX-03C is the next slice for recent raw complete Exchanges plus current-Segment-only semantic recall of older relevant complete Exchanges, ending in a final capacity guardrail and one primary Tutor call. Excludes prior-Segment reopening, prior-session/archive retrieval, memory service, retro-linking, extra classifier/summarizer calls, and external vector databases. | CTX-03C is READY. CTX-03D/E remain NOT STARTED. |
...
| **CTX-03C** | READY | Recent raw complete Exchange context plus current-Segment-only semantic recall of older relevant complete Exchanges through existing PostgreSQL/pgvector embedding infrastructure. Full Immediate Exchange remains protected; relevance is structural and capacity is only the final guardrail. |
...
**Next action:** Independent review, then CTX-03C — Recent Raw Context +
Segment-Scoped Semantic Recall. Do not begin REC-25 or LR-D04B.
