...
`division_workspace`. The correction itself did not promote a renderer; the
later separately authorized `MATH-RENDER-NUMBER-LINE-01` child promotion is
recorded in Section 14.4.1.
...
**Status:** DONE / ACCEPTED on 2026-09-07 — bounded child only. The Product Owner first promoted
this child of the unaccepted `MATH-RENDER-BATCH-01` umbrella. It delivers
...
`G5-NBT-DECIMAL-ROUND`). It is not family completion and does not change the
accepted planning-pack implemented-coverage count of 0/36.

The later implementation must resolve the exact profile/activity/renderer/
seed/action/event/validator/reducer versions; finite numerical domain and
precision; exact representation/serialization/comparison; midpoint/tie policy;
grid endpoints, snapping, equality and boundaries; equivalent decimal forms;
initial/pending/submitted completeness; safe configuration/feedback projection;
and the existing server-owned fixture/configuration activation path. It must
use server-authored exact operands, scales, point identities and endpoints;
declared-grid placement/movement or selection; bounded relation/rounding
selection; and explicit submission. Pixel coordinates only identify candidate
grid positions; server validation decides exact mathematical state. Numeric
order stays LTR inside RTL surrounding prose. Free-form explanation remains
Chat.

Use the accepted exact-version Subject Capability Registry, activation/safe
seed, Event/Snapshot/rebuild, protocol/controller/feed, `/student/daily`
Renderer Host and Runtime-03 provenance. Exploration is `RECORD_ONLY`; only
the declared submission triggers one Tutor continuation. Preserve source-linked
StudentInteraction, original submission, identity/ownership/Safety/
LiveSubjectContext and version/feed invariants; no direct Canvas intelligence
writes. Reject malformed, out-of-domain, stale, cross-Student and state-
mismatched requests without mutation; record bounded truthful feedback for
structurally valid but incorrect submissions. Do not add a generic generator,
extra model call, Router, generic artifact engine, browser durable reducer,
...
typecheck/build, proportional full suite and fresh independent review. Direct
API calls and isolated mounts are insufficient. Powers-of-ten, estimates,
fraction modes and all other number-line or renderer-family scope remain NOT
PROMOTED.
...
server-owned operands, typed place counts, quantity-preserving exchanges, a
bounded written result, and explicit submission across multiple authored
configurations. It is not a hard-coded example or a general problem generator.

Before production code, the task must define finite operand/result ranges;
supported columns/precision; exact integer-hundredths or equivalent exact
representation/serialization; immutable operands versus mutable model/result;
exchange/combine/removal/completion and non-normalized-count limits; input
syntax/locale/equivalent-form equality; and configuration identity/provenance/
safe-seed activation. Number-line thousandths and range are not inherited.
Every exchange conserves quantity; every combine/removal retains arithmetic
accounting and operand lineage. Valid pre-regrouping counts must not be rejected
merely for being outside 0–9. Over-removal, negative/out-of-range/fabricated
quantity reject without mutation, while structurally valid wrong work receives
bounded feedback. The contract must cover regrouping, subtraction decomposition
through zero, zero/equal operands, carry into a supported additional place, and
alternative valid exchange sequences.

Reuse exact Subject Capability versions, server-owned activation/configuration,
safe immutable problem identity, Studio Event/Snapshot/rebuild/feed, existing
`/student/daily` Renderer Host/controller, and Runtime-03. Preserve historical
MATH profiles, accepted number-line availability, and Make-Ten Host support.
Exploration/result editing are RECORD_ONLY; only explicit submission triggers
one interaction and one Tutor continuation. Preserve original submission versus
later Snapshot; no fake Student message or direct Canvas intelligence write.
Do not add a second Router/session/feed/reducer/Tutor/Canvas Specialist,
generic artifact engine, broad FE-02 refactor, dependency, schema, or migration
by default. Browser problem/answer authoring and Tutor-prose parsing are out.

The eventual React/DOM/SVG/local workspace must make columns, operation,
quantities/equivalent exchanges, attempted result, recovery, and separate
submission readable. It requires pointer and genuinely evidenced emulated touch,
keyboard/buttons, focus, cancellation/retry, reconciliation, narrow/reduced-
motion/text alternatives; math ordering remains independent of surrounding RTL
prose. Backend acceptance covers both operations/configurations, exact
invariants, zero/boundary/regrouping alternatives, wrong-valid submissions,
rejection without mutation, activation/idempotency/reload/rebuild/version/feed,
one Runtime-03 continuation, original/later state distinction, and shared
activity regressions. Authenticated `/student/daily` browser acceptance
...
Voice/Vision, image annotation, NotebookLM, Replit, deployment, real-Lina use,
and research mapping/coverage changes remain NOT PROMOTED.
...
**Output:** an exact-version Science Activity/Renderer/fixture contract with
stable typed stages; deterministic fixture-owned sequence/relationship
validation; record-only reordering; exactly one contract-declared submitted
configuration interaction; durable Event/Snapshot replay; and one Runtime-03
Tutor continuation from the exact submission. Reuse the existing Registry,
state/protocol/feed, interaction, and web-controller seams. The renderer is a
minimal isolated review seam, not a `/student/daily` host.
**Implementation checks:** retain empty `SCIENCE / subject-profile-v1` for
replay and declare all new exact version literals; record fixture provenance,
stage identities, order/relationship and alternative-order policy, payloads,
activation identity, and interaction design. The answer key remains
server-owned rather than browser-visible Scene seed data unless separately
approved. Any cross-activity activation/supersession behavior must be explicit
and tested; the current Make-Ten adapter is only an exact-activity reference.
**Verification:** exact registry/activation identity; valid, invalid,
alternative-valid-if-declared, duplicate, missing, extra, stale, replay, and
cross-Student operation behavior; reload/rebuild equality; submitted
configuration distinct from later current state in the Tutor context; one
Runtime-03 continuation with no fake Student message, direct intelligence
write, or additional Canvas call; and isolated pointer, emulated touch,
keyboard/button, accessibility, locale/direction, narrow-layout, rejection,
and reduced-motion review. Independent review is required before acceptance.
**Non-scope:** generic Artifact Engine/drag framework, full Science curriculum,
...
`TUTOR_TRIGGERING` action. The server-owned answer key validates the submitted
...
truth, or Canvas language-safety system. Resolve exact profile/activity/
...
behavior, submitted-versus-later-current state, and no fake message/direct
writes/extra Canvas call. Fresh browser review covers pointer, touch,
...
**Replace:** prototype prose/equation parsing, message-derived Canvas,
duplicated placeholder cards, and browser-only Studio truth.
**Add:** the existing project-owned Studio controller at the Daily App,
initial Snapshot/load/resume behavior, an exact-contract active-Scene Renderer
Host for accepted Math/Science/English Activities, optimistic-operation
presentation, pending/error/reconnect states, and Runtime-03 continuation
composition.
**Boundary:** `/student/daily` is the production target. Do not import, wrap,
extract from, restyle, modify, or route through `/student` or
`StudentMathSession`; do not merge the preserved prototype. Browser state is
presentation/optimistic only. Tutor SSE remains Chat authority and the
dedicated Studio feed remains Workspace authority.
**Execution record:** the current detailed executable task, exact reusable
backend/web seams, pre-code checks, non-scope, and required authenticated
verification are in `project-state/DAILY_USE_RELEASE_TASKS.md` under this task.
**Verification:** authenticated production-intent Chat-only, Math, Science,
English, streaming/concurrency, reconnect/recovery, server-rejection,
desktop/narrow, RTL/LTR/mixed-direction, keyboard/focus, reduced-motion, and
error flows; focused integration tests, web typecheck/build, relevant Python
regressions, proportional full-suite regression, diff checks, and fresh
independent review. Isolated review mounts are insufficient.
...
No Replit experiment or deployment work is authorized while the Grade 5 Math
planning correction, later renderer tasks, and full-system acceptance remain
pending. Replit is blocked in this promotion.
