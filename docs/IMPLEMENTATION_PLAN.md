...
until promoted through the current task/state process.

> **`IMPLEMENTATION_PLAN.md` defines the execution architecture. `PROJECT_STATE.md` defines current operational reality. `TASKS.md` defines executable task state/history.**

Historical build-order sections in this plan remain provenance and architectural sequencing context. They do not override a newer Product Owner-approved current task in `PROJECT_STATE.md` / `TASKS.md`.
...
The first objective is to prove and operate one real end-to-end learning loop with Lina.
...
Closed Segments with valid lineage and persisted Student raw interaction receive semantic review
        ↓
Deterministic Session Intelligence Finalization authorizes Event/Evidence
        ↓
...
The core technical/intelligence architecture above is implemented and accepted. Limited real Lina interaction has also occurred. The remaining product/operational objective is to turn that limited use into a reliable recurring private Lina experience and then expand approved capabilities deliberately rather than rebuilding the accepted core.
...
Implementation must follow this authority order by document responsibility:
...
the relevant item is explicitly promoted through current task/state governance.
...
## 3.7 Actual Work Queue / Durable Task State

`TASKS.md`

This is what Codex should execute together with any explicit current Product Owner-approved transition recorded in `PROJECT_STATE.md`.
...
Operational activation of an already implemented process is not, by itself, evidence that the project needs a deployment redesign, new queue, new database, or new infrastructure service.
...
Approved direction, implementation-gated:
...
The technology direction does not itself unfreeze Learning Canvas or the Artifact Engine.
...
Approved domain direction. When explicitly promoted, owns:
...
Do not treat this domain description as current implementation authorization.
...
```text
execute(task="tutor", ...)
execute(task="segment_evidence", ...)  # CURRENT Segment Review route
execute(task="session_evidence", ...)  # LEGACY / HISTORICAL compatibility
execute(task="vision_student_work", ...)  # FUTURE / GATED
```

## 7.7 Grade Domain

Approved product direction; production expansion remains gated. Owns when promoted:
...
Initial intended subjects:

- Math
- Science

Current production proving ground is Math-first. Science production remains explicitly sequenced/gated rather than automatically executable.
...
When the Artifact capability is promoted, a new artifact should primarily require:
...
Recommended initial/logical repository layout:
...
This is a logical organization target, not a command to create unnecessary package boundaries or frozen capability modules prematurely.
...
Current accepted/implemented concepts include:

- Segment Learning Reviews,
- Segment Review processing/version lineage,
- multi-message Event provenance,
- Event Segment lineage,
- optional candidate-event source metadata where persisted,
- learning events,
- evidence,
- current states,
- learner patterns,
- evidence/pattern links,
- Session intelligence authority/processing generations,
- intelligence snapshots/decision views as implemented.

Historical legacy Session Evidence remains valid audit/compatibility history; it is not the current primary semantic review architecture.

## 11.5 Grade Transition

- compact Grade transition cards — approved future production direction
...
When validating Math RAG against a real book/source, maintain a small evaluation set containing:
...
A real book is useful validation input, not a Tutor permission gate.
...
The accepted DEC/REP/LANG/CAND Tutor-calibration work governs the current runtime. Historical REC-35.2 wording remains provenance; do not reopen its implementation merely because this plan describes the underlying design.

The same primary Tutor call decides Mode, Strategy, Method, and prior-method relation from the current message, relevant context/personalization, prior persisted method, and compact taxonomies. Runtime validates allowed values, null combinations, frozen status, source lineage, and cross-field consistency; it must not use keyword/phrase lists to infer meanings or make a separate classification call.

Turn decisions are routing/audit metadata, not learner memory. Selecting or using a TeachingMethod is not Evidence that it worked or failed. Effectiveness is evaluable only after an observable Student outcome through optional provisional Candidate hints, completed Segment Review interpretation, and Session-authorized Evidence.
...
The normal path uses one primary Tutor-model call per turn.
...
CTX-03A through CTX-03E describe the implemented/verified technical context architecture. Browser and recurring Real-Lina validation remain separate evidence categories. Historical archive retrieval remains a deferred on-demand seam; no separate conversation classifier, extra summarizer model call, archive vector index, automatic prior-session semantic retrieval, retro-link background job, memory service, or agent chain is authorized by this plan.

# 15. Multimodal Input Architecture

Student interaction is intended to become multimodal-first rather than remain chat-only. Production Voice/Vision work is approved product direction but remains sequencing-gated until explicitly promoted.
...
Pipeline when Vision is promoted:
...
When visual feedback is implemented and useful:
...
Interactive educational output is an approved core product capability, currently implementation-gated.
...
## 16.3 Initial Renderer Direction
...
These are approved renderer candidates/direction, not authorization to build the full Artifact Engine during an unrelated track.
...
This is the implemented/accepted current authority model.

## 17.4 Normal AI Cost Pattern

Current target runtime pattern:
...
The currently activated books/content may ground the current Grade learning environment; Tutor availability itself is independent of book availability.

## 19.2 Grade Transition

Approved future production direction:

```text
Grade 5 active environment
      ↓
Parent/Admin activates Grade 6 books/context
      ↓
...
The policy engine is an explicit runtime contract. Its category-detection implementation may use deterministic routing and/or the same primary Tutor semantic output where accepted, but the effective policy action is governed by versioned system/Parent policy. Do not rely on one Tutor prompt as the only enforcement mechanism.
...
Approved future/gated task categories may include:
...
Roadmap Track B, not an automatic current implementation task.
...
Start/use a fast, cost-efficient model candidate for normal teaching and benchmark it on Lina-specific teaching scenarios.

Do not make a specific provider/model an architectural dependency. The current real route may use OpenAI; that is an operational implementation choice behind Model Gateway.
...
- historical current-focus extraction where compatibility requires it

### Optional/rare

- curriculum semantics / full book semantic reprocessing
...
- transcripts,
- student images when implemented,
- Segment Reviews/findings,
- events,
- evidence,
- pattern changes,
- Tutor adaptations,
...
- Vision interpretation when implemented.

Validate both schema and key semantic expectations.

## 24.3 Content/Retrieval Golden Set

Use Lina's real Math book or other real source when available to maintain expected retrieval cases. Content is a validation/grounding input, not a permission gate for Tutor use.
...
- retention failure,
- drawing/handwriting evidence when implemented,
- multi-thread session,
- Grade transition when implemented.
...
Limited real Lina use has already occurred and contributed to calibration. Keep it distinct from stable recurring/daily use and longitudinal validation.

Review recurring real use for:

- whether Lina wants to use the system,
- whether explanations help,
- whether RAG is correctly grounded when sources exist,
- whether Evidence matches interactions/work,
- whether Patterns are useful,
- whether personalization improves later interaction,
- whether approved multimodal/interactive capabilities should be promoted based on actual friction/need.
...
Actual executable work must be decomposed/promoted through `TASKS.md` and current state. Historical phase completion/ordering does not by itself create a current task.
...
> dependency as a permission gate: the Tutor works with zero content, and
...
- early Lina calibration evidence is reviewed distinctly from later recurring/daily and longitudinal validation.
...
**Historical implementation note:** Phase 3 records completed legacy/current
Session Evidence work factually. The later SEG-EVID correction is now implemented/accepted and governs the current semantic authority boundary; this does not erase completed historical Session Evidence work.
...
- historical Session Evidence consolidation compatibility,
- implemented Segment Review + deterministic Session Finalization authority,
- validated Learning Events,
...
A meaningful Math session creates traceable Evidence and current intelligence, and a later Tutor session can use relevant intelligence without loading full history.
...
The Segment-scoped semantic Review + Session-scoped durable authority architecture has passed Full-System Learning Intelligence Acceptance. Recurring natural Lina use remains a separate validation horizon.
...
# 26. Real Lina Decision Gate — Updated Verification Horizon

Limited real Lina use has already occurred. Therefore this gate no longer asks only whether Lina has ever used the product.

The remaining mandatory product/operational question is whether the system becomes a **reliable recurring Daily-Use Lina Baseline** and whether the differentiated intelligence loop behaves credibly across natural sessions. First Product Loop completion remains broader and includes sufficient Parent inspectability rather than being declared complete from Student chat alone.

## 26.1 Required Real Inputs

- natural Grade 5 Math use across multiple Lina sessions,
- real school/source material when useful, but not as a Tutor prerequisite,
- sufficient elapsed/use diversity to inspect cross-session intelligence behavior.
...
- Does text-only interaction become a recurring friction that should promote Voice/Vision/visual artifacts?

### Content/Retrieval

- Does grounding improve school-related teaching when useful sources exist?
- Does zero-source behavior remain useful?
- Are important examples/figures accessible when content is available?

### Learning Intelligence

- Are meaningful Events actually meaningful?
- Does Evidence match the interaction/work?
- Do Patterns make sense under the approved rules?
- Does old intelligence resolve when Lina improves?
- Does personalization improve later interactions without fighting current behavior?

### Parent Experience

- Can the Parent inspect and understand important system reasoning?
- What Parent visibility is actually needed to complete the first product loop?
...
Core loop is useful and trustworthy enough to promote the next approved capability deliberately.

### Continue With Corrections

Fix Tutor, RAG, Evidence, Pattern Rules, UX, or operations before broader expansion.
...
Explicit promotion based on product sequencing/real-use evidence and stable intelligence contracts.
...
A bounded Math readability defect such as MATH-01 does not by itself unfreeze this phase.
...
**Status:** Approved core product direction / production implementation frozen until explicit promotion.
...
Validated core + explicit Product Owner promotion.
...
**Status:** Approved future/evidence-dependent direction.
...
**Status:** Approved direction / production implementation deferred.
...
- Grade changes only when Parent/Admin activates new Grade content/context,
- Grade 6 begins from Grade 6 context,
...
When promoted, use Vision when Lina provides a relevant image or when educational value requires it.

## 33.4 Artifacts

When promoted, prefer reusable deterministic renderers over generating new HTML/JS from a model every time.
...
- deployment redesign solely to activate existing Web/API/Worker/DB/model wiring.
...
The first product loop is broader than “Student Tutor works” and is complete only when all of the following are true.
...
- recurring real use is natural enough to justify the product loop; approved multimodal/visual capabilities remain separate promotion decisions unless evidence shows they are needed to reach that baseline.
...
- naturally recurring use validates that personalization helps without fighting current behavior.

## Parent

- Parent can see important learning state.
- Parent can open evidence behind important conclusions.
- Parent can manage useful content/current Grade context.
...
- limited Real-Lina use is preserved as historical evidence,
- stable recurring Lina use and the core cross-session intelligence loop are reviewed at the current real-use gate,
- first product loop is not declared complete solely because Text/Math Tutor works.

---

# 37. Current Execution Authority

The historical build sequence in this plan is **not** the current task queue.

Do not execute old entries such as Track A, SEG-EVID, SCOPE-01, SUBJ-01, or completed Tutor calibration tasks merely because they appear earlier in this document. Their accepted outcomes are architecture/history.

For the current next executable track, always read:
...
The following remain useful product gates even though earlier technical phases are already implemented/accepted.

## Gate A — Content Reliability

Before the Tutor relies on school RAG for a specific grounded claim, useful real-source retrieval must be demonstrably reliable. The Tutor itself does not require content readiness.

## Gate B — Intelligence Traceability

Before historical personalization is treated as reliable product behavior, Events/Evidence/Patterns must remain inspectable and rebuildable. Technical Full-System Acceptance has established the architecture; recurring natural Lina validation remains separate.

## Gate C — Daily-Use Lina / Capability Promotion

Limited Real-Lina use has occurred. Before broadly expanding frozen capability tracks, verify the recurring private Lina core sufficiently to understand the highest-value next need, then require explicit Product Owner promotion. Real use orders approved Voice/Vision/Science/Artifact/Parent capability work; it does not erase those approved directions.

## Gate D — Grade Transition

Do not build elaborate Grade transition behavior before the current Grade product is proven and the transition capability is explicitly promoted.
...
> **Preserve the smallest complete accepted learning loop. Operate and validate it in recurring real use. Expand approved capabilities only when evidence and Product Owner sequencing show where additional complexity creates value.**

The correct implementation is not the one that implements the largest number of approved ideas fastest.

It is the one that keeps the core learning loop coherent, observable, rebuildable, child-usable, and easy to evolve.
