...
- Rename the configurable restricted category to `SEXUAL_CONTENT`; migrate existing `HUMAN_REPRODUCTION` boundary rows only, leaving historical audit rows untouched.
...
3. Refactor safety policy into hard-baseline evaluation plus server-owned Parent Boundary settings/resolution. Remove `_CATEGORY_TERMS`; add the reversible category migration.
