...
import { RelationFocusView as MotionProof, SpatialPlacement as KonvaProof, CoordinateConstruction as JsxGraphProof, MathExpressionInput as MathLiveProof } from '../../lib/studio/visual-toolbelt';
import Konva from 'konva';
import JXG from 'jsxgraph';
Object.assign(window, { spatialPositions: () => ({ circle: Konva.stages[0].findOne("Circle")!.getAbsolutePosition(), label: Konva.stages[0].find("Text").find(node => (node as Konva.Text).text() === "A")!.getAbsolutePosition() }), engineCounts: () => ({ stages: Konva.stages.length, boards: Object.keys(JXG.boards as Record<string, unknown>).length }) });
function Fixture() {
  ...
  const [placement, setPlacement] = useState({ objectId: 'object-a' as const, targetId: null as 'target-b' | null });
