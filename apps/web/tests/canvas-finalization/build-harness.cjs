// Isolated build of the actual final Canvas review; no auth, Tutor or persistence imports.
const fs = require("node:fs");
const path = require("node:path");
const root = path.resolve(__dirname, "../../../..");
const out = path.resolve(process.env.CANVAS_FINAL_HARNESS || "/tmp/lina-canvas-final");
...
webpack.init();
webpack.webpack({
  // Sucrase emits the development JSX runtime in this isolated harness.
  // Production capability is verified separately by the real Next build.
  mode: "development",
  devtool: false,
  ...
  const css = [
    path.join(root, "apps/web/components/studio/canvas-final-review.css"),
    path.join(root, "apps/web/lib/studio/visual-toolbelt/toolbelt.css"),
