...
export function ProcessView({scene,state,onAction,onExplain,reducedMotion=false}:{scene:ProcessScene;state:ProcessViewState;onAction:(action:ProcessViewAction)=>void;onExplain:(id:string)=>void;reducedMotion?:boolean}) {
 ...
 return <section className="pv" data-reduced={reducedMotion} dir={ar?'rtl':'ltr'} lang={scene.locale} aria-label={scene.title}>
 <style>{styles}</style>
 ...
 const returning=returnRelation?.id===r.id,emphasized=state.highlightedRelationIds?.includes(r.id),traced=state.tracingRelationId===r.id;
