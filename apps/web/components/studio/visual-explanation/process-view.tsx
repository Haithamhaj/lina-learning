...
export function ProcessView({scene,state,onAction,onExplain,reducedMotion=false}:{scene:ProcessScene;state:ProcessViewState;onAction:(action:ProcessViewAction)=>void;onExplain:(id:string)=>void;reducedMotion?:boolean}) {
 ...
 const returning=returnRelation?.id===r.id,emphasized=state.highlightedRelationIds?.includes(r.id),traced=state.tracingRelationId===r.id||motion.has('TRACE_SEQUENCE')||motion.has('TRACE_CYCLE');
 return <g key={r.id} data-relation={r.id} data-motion-order={traced?i:undefined} className={`pv-relation ${returning?'pv-return':''} ${emphasized?'pv-relation-active':''} ${traced?'pv-motion-trace':''}`} style={traced&&!reducedMotion?{animationDelay:`${i*180}ms`}:undefined}>
