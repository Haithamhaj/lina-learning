...
export function ProcessView({scene,state,onAction,onExplain,reducedMotion=false}:{scene:ProcessScene;state:ProcessViewState;onAction:(action:ProcessViewAction)=>void;onExplain:(id:string)=>void;reducedMotion?:boolean}) {
 ...
 const returning=returnRelation?.id===r.id,emphasized=state.highlightedRelationIds?.includes(r.id),motionEmphasis=emphasized&&motion.has('EMPHASIZE_RELATION'),traced=state.tracingRelationId===r.id||motion.has('TRACE_SEQUENCE')||motion.has('TRACE_CYCLE');
 return <g key={r.id} data-relation={r.id} data-motion-order={traced?i:undefined} className={`pv-relation ${returning?'pv-return':''} ${emphasized?'pv-relation-active':''} ${motionEmphasis?'pv-motion-emphasis':''} ${traced?'pv-motion-trace':''}`} style={traced&&!reducedMotion?{animationDelay:`${i*180}ms`}:undefined}>
 ...
 const p=positions[i],emphasized=state.highlightedStageIds?.includes(stage.id)||focus===stage.id;
 const labelTop=compact?p.y+72:scene.topology==='cycle'&&i===0?p.y-110-topExtra:p.y+78;
 return <g key={stage.id} data-stage={stage.id} data-motion-order={motion.has('REVEAL_IN_ORDER')?i:undefined} role="button" tabIndex={0} aria-pressed={state.selectedId===stage.id} aria-label={`${i+1}. ${stage.label}`} className={`pv-stage ${emphasized?'pv-stage-active':''} ${focus&&!emphasized?'pv-stage-muted':''} ${motion.has('REVEAL_IN_ORDER')?'pv-motion-reveal':''} ${motion.has('TRANSITION_FOCUS')&&focus===stage.id?'pv-motion-focus':''}`} style={motion.has('REVEAL_IN_ORDER')&&!reducedMotion?{animationDelay:`${i*120}ms`}:undefined} onClick={()=>onAction({type:'focus',id:stage.id})} onFocus={()=>{if(focus!==stage.id)onAction({type:'focus',id:stage.id});}} onKeyDown={e=>moveFocus(e,i)}>
