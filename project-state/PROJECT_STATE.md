...
Hybrid Segment Context is the APPROVED Context v2 architecture. Parent-owned Student Core Profile is an APPROVED FUTURE FOUNDATION, not implemented, and belongs to future TASK-027A. REC-35.2 / LR-D04A remains in REVIEW behind the mandatory Lina Stabilization Gate. CTX-01, ACT-01, OBS-01, UI-01, CTX-02, SAFE-01, SAFE-02, OUT-01, and DATA-01 are CLOSED. CTX-03B is ACCEPTED after its long real-Luna replay; CTX-03C is READY as the next implementation slice. CTX-03D/E are NOT STARTED. LR-D04B remains deferred and REC-25 remains BLOCKED.
...
- CTX-03C is READY: one primary Tutor call will receive the Current Multimodal Turn, Full Immediate Exchange, a small calibration-sized Recent Raw window of complete Exchanges, Compact Structured Segment State, and current-Segment-only semantic recall of older relevant complete Exchanges, then a final capacity guardrail. Relevance is structural; State references, vector relevance, and recency are late tie-breaks. Zero semantic matches is valid. Current Turn remains unassigned before Luna returns its Segment relation; no prior Segment/session/archive retrieval, second classifier/summarizer call, or external vector database is approved.
...
- CTX-03C is the next READY slice. It must reuse PostgreSQL, pgvector, `ModelTask.EMBEDDING`, the OpenAI embedding provider, `text-embedding-3-small` at 1536 dimensions, the model execution ledger, and embedding batching; conversation-memory indexing remains Tutor/session-owned rather than Curriculum RetrievalService authority. A precomputed query-vector seam is optional later work. Archive retrieval, automatic retro-linking, memory service, extra classifier/summarizer model calls, and an external vector database remain deferred.
...
Independent review, then CTX-03C implementation. Do not begin CTX-03D/E, ACT-02, REC-25, LR-D04B, archive retrieval, or frozen future capabilities. TASK-027A remains future/blocked.
