...
- LANG-01 is REVIEW / IMPLEMENTED + AUTOMATED VERIFIED + REAL-LUNA VERIFIED. One compact shared Tutor instruction keeps a language-neutral current turn—number, fraction, equation, mathematical expression, or answer-choice symbol—in the established primary language from the immediate active exchange/current context. It explicitly forbids choosing Arabic or English from neutral notation alone, preserves clear current-language switches, and permits natural bilingual school/math terminology. There is no language detector, router, persistent language state, or extra model call. Automated verification is CODEX-REPORTED; bounded real `openai/gpt-5.6-luna` verification is CODEX-REPORTED REAL LUNA VERIFIED. Browser and Real-Lina remain NOT VERIFIED. Independent GitHub / Product Owner review is required before acceptance.
...
Independent GitHub / Product Owner review of `LANG-01 — Language Continuity` is the next required action. Do not begin `CAND-02` before that review.
